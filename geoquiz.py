...
def color_country(country_name):
    if country_name in world['ADMIN'].values:
        world['color'] = world['ADMIN'].map(lambda x: 'red' if x == country_name else 'white')
        ...
        centroid = world.loc[world['ADMIN'] == country_name, 'geometry'].centroid.values[0]
